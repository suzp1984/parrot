...
#define XMLNODE_FILE "./test_xmlnode.xml"

int main(int argc, char* argv[])
{
	xmlnode* root;

	if (!(root = xmlnode_root_from_file(XMLNODE_FILE))) {
		const gchar* root_string = "<xmlnode></xmlnode>";
		root = xmlnode_root(root_string, strlen(root_string));
	}

	xmlnode_new_child(root, "accounts");
	xmlnode_save_file(root, XMLNODE_FILE);
