...
#endif
    {
        LOG("thread", "WARNING: thread create failed\n");
        return false;
    }
...
void Mutex::unlock()
{
    if (!ReleaseMutex((HANDLE) mState))
        LOG("thread", "WARNING: bad result from unlocking mutex\n");
}
...
status_t Mutex::tryLock()
{
    ...
    if (dwWaitResult != WAIT_OBJECT_0 && dwWaitResult != WAIT_TIMEOUT)
        LOG("thread", "WARNING: bad result from try-locking mutex\n");
